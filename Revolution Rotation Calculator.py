...
Abilities = ['ASPHYXIATE', 'ASSAULT', 'BACKHAND', 'BARGE', 'BERSERK',
             ...
             "DEATH'S SWIFTNESS", 'DEBILITATE', 'DECIMATE',
             'DEEP IMPACT', 'DESTROY', 'DETONATE', 'DISMEMBER', 'DRAGON BREATH', 'FLURRY', 'FORCEFUL BACKHAND',
             'FRAGMENTATION SHOT', 'FRENZY', 'FURY',
             'HAVOC', 'HURRICANE', 'IMPACT', 'KICK', 'MASSACRE', 'METAMORPHOSIS', 'NEEDLE STRIKE', 'OMNIPOWER',
             'ONSLAUGHT', 'OVERPOWER', 'PIERCING SHOT',
             'PULVERISE', 'PUNISH', 'QUAKE', 'RAPID FIRE', 'RICOCHET', 'SACRIFICE', 'SEVER', 'SHADOW TENDRILS',
             'SHATTER', 'SLAUGHTER', 'SLICE', 'SMASH',
             'SMOKE TENDRILS', 'SNAP SHOT', 'SNIPE', 'SONIC WAVE', 'STOMP', 'STORM SHARDS', 'SUNSHINE',
             'TIGHT BINDINGS', 'TSUNAMI', "TUSKA'S WRATH", 'UNLOAD',
             'WILD MAGIC', 'WRACK']
# --- Defining how abilities work --- #
AttackSpeedCooldowns = {'FASTEST': 2.4, 'FAST': 3.0, 'AVERAGE': 3.6, 'SLOW': 4.2,
                        'SLOWEST': 7.2}  # Cooldowns for casewhere no abilities may be used
AbilityDamage = {'DEBILITATE': 60, 'UNLOAD': 610, 'TIGHT BINDINGS': 120, 'SNIPE': 172, 'SNAP SHOT': 265,
                 ...
                 'NEEDLE STRIKE': 94.2, 'FRAGMENTATION SHOT': 241.2, "DEATH'S SWIFTNESS": 0, 'DEADSHOT': 426.13,
                 ...
                 'DEEP IMPACT': 120, 'CORRUPTION BLAST': 200, 'CONCENTRATED BLAST': 152.8, 'COMBUST': 241.2,
                 ...
                 'SLAUGHTER': 435, 'SLICE': 75, 'SMASH': 94.2, 'ASSAULT': 525.6, 'DECIMATE': 112.8, 'DISMEMBER': 120.6,
                 'FURY': 152.8, 'KICK': 60, 'PUNISH': 56.4, 'QUAKE': 131.4,
                 'STOMP': 120}  # Ability damage of every ability
AbilityCooldown = {'DEBILITATE': 30, 'UNLOAD': 60, 'TIGHT BINDINGS': 15, 'SNIPE': 10.2, 'SNAP SHOT': 20.4,
                   ...
                   'PUNISH': 3, 'QUAKE': 20.4, 'STOMP': 15}  # Cooldowns of abilties (in seconds)
AbilityType = {'DEBILITATE': 'T', 'UNLOAD': 'U', 'TIGHT BINDINGS': 'T', 'SNIPE': 'B', 'SNAP SHOT': 'T',
               ...
               'STOMP': 'T', 'MASSACRE': 'U'}  # Type of ability (B = basic, T = threshold, U = ultimate)
Ready = {'DEBILITATE': False, 'UNLOAD': False, 'TIGHT BINDINGS': False, 'SNIPE': True, 'SNAP SHOT': False,
         ...
         'MASSACRE': False}  # Flag on if you can use abilties (based on adrenaline)
Bleeds = {'SHADOW TENDRILS': 1.8, 'FRAGMENTATION SHOT': 6, 'DEADSHOT': 6, 'CORRUPTION SHOT': 6, 'SMOKE TENDRILS': 5.4,
          ...
          'MASSACRE': 6}  # time DOT abilties last (seconds)
SpecialBleeds = ['MASSACRE', 'DEADSHOT']  # Bleeds that have their first hit affected by damage modifying abilities
SpecialAbilities = ['DEEP IMPACT',
                    'SNIPE']  # Abilities that take longer than 1.8 seconds to use but will still have full impact from abilties in list CritBoost
Buff_Time = {'BARGE': 6.6, 'FURY': 5.4, 'RAPID FIRE': 6, 'TIGHT BINDINGS': 9.6, 'BINDING SHOT': 9.6,
             ...
             'CONCENTRATED BLAST': 5.4, 'BERSERK': 19.8, 'FORCEFUL BACKHAND': 3.6,
             'STOMP': 3.6}  # How long stuns, DPS increases .. etc last
Buff_Effect = {'NEEDLE STRIKE': 1.07, "DEATH'S SWIFTNESS": 1.5, 'SUNSHINE': 1.5, 'METAMORPHOSIS': 1.625, 'FURY': 1.1,
               'CONCENTRATED BLAST': 1.1, 'BERSERK': 2, 'SLICE': 1.506, 'PUNISH': 2, 'WRACK': 2,
               'PIERCING SHOT': 2}  # multiplier for boosted damage
...
Debilitating = ['BARGE', 'FORCEFUL BACKHAND', 'STOMP', 'DEEP IMPACT', 'BINDING SHOT', 'TIGHT BINDINGS',
                'RAPID FIRE']  # Abilities that can stun or bind target
...
def AbilityRotation(Permutation, AttackSpeed, Activate_Bleeds, Ring, Start_Adrenaline, Auto_Adrenaline,
                    Time):  # Will return how much damage an ability bar will do over a given time
    # --- Defining Variables --- #
    ...
    AbilityPath.append(
        'AUTO' + ' D: ' + str(round(Current, 1)) + ' T: ' + str(round(Clock, 1)) + ' A: ' + str(Adrenaline))
    Current += 50
        ...
        for ability in Permutation:
            if Clock < Time:
                if Ready[ability] == True:  # Checks if ability can be used
                    ...
                    AbilityPath.append(
                        ability + ' D: ' + str(round(Current, 1)) + ' T: ' + str(round(Clock, 1)) + ' A: ' + str(
                            Adrenaline))
                    ...
                    if ability in Bleeds:
                        ...
                        if (ability in Punishing) and (Buff_Available() == True):
                            if (Time - Clock) >= AbilityTime[ability]:
                                ...
                                Current += round(AbilityDamage[ability] * Buff_Effect[ability] * Current_Buff * (
                                (Time - Clock) / AbilityTime[ability]), 1)

                        else:
                            if (Warning == True) and (Current_Buff > 1) and (AbilityTime[ability] > 1.8) and (
                                ability not in SpecialAbilities):
                                if (Time - Clock) >= AbilityTime[ability]:
                                    Current += round(AbilityDamage[ability] * (((Current_Buff - 1) * Multiplier) + 1),
                                                     1)
                                else:
                                    Current += round(
                                        AbilityDamage[ability] * (((Current_Buff - 1) * Multiplier) + 1) * (
                                        (Time - Clock) / AbilityTime[ability]), 1)
                            else:
                                if (Time - Clock) >= AbilityTime[ability]:
                                    ...
                                    Current += round(
                                        AbilityDamage[ability] * Current_Buff * ((Time - Clock) / AbilityTime[ability]),
                                        1)
                    ...
                    Current_Buff = AdjustCooldowns(Current_Buff, Adrenaline,
                                                   AbilityTime[ability])  # Will also manage cooldowns
                    break
            ...
            if AbilityAvailable == False:
                if Auto_Available() == True:
                    if (Clock + AttackSpeedCooldowns[AttackSpeed]) <= Time:
                        ...
                        break
                    AbilityPath.append(
                        'AUTO' + ' D: ' + str(round(Current, 1)) + ' T: ' + str(round(Clock, 1)) + ' A: ' + str(
                            Adrenaline))
                    ...
                    if Adrenaline > 100:
                        Adrenaline = 100
                    Current_Buff = AdjustCooldowns(Current_Buff, Adrenaline, (
                    AttackSpeedCooldowns[AttackSpeed] + 0.6))  # Will also manage cooldowns
                else:
                    Clock += 0.6
                    Clock = round(Clock, 1)
...
def Get_Time(Seconds):  # Converts raw seconds into Years, Weeks, etc...
    ...
    Seconds -= Minutes * 60
    Time = str(Years) + ' years, ' + str(Weeks) + ' weeks, ' + str(Days) + ' days, ' + str(Hours) + ' hours, ' + str(
...
def AdjustCooldowns(Current_Buff, Adrenaline,
                    Time):  # Decreases Cooldowns of abilities and buffs as well as modifying and damage multipliers
    for Ability in TrackCooldown:
...
AbilityTime = {'DEBILITATE': 1.8, 'UNLOAD': 4.2, 'TIGHT BINDINGS': 1.8, 'SNIPE': 3.6, 'SNAP SHOT': 1.8,
               ...
               'STOMP': 1.8}  # How long it takes to use each ability
CopyOfReady = {}
...
while True:
    try:
        Time = float(input(
            'How long will rotation last? WARNING: Longer times require longer wait times, a better processor will improve this speed. \n>> '))
        break
    except:
        Error()
...
try:  # Will keep running until Control C (or other) is pressed to end process
    for index in range(0, Permutations):
        ...
        Current = AbilityRotation(Permutation, AttackSpeed, Activate_Bleeds, Ring, Start_Adrenaline, Auto_Adrenaline,
                                  Time)
        ...
        if Runthrough % 10000 == 0:
            print(str('\r===== ') + str(round(float(Runthrough / Permutations) * 100, 3)) + str('% ===== ') + str(
                'Estimated time remaining: ') + str(
                Get_Time(int(End_Estimation - (time.time() - Start)))) + '; Best found: ' + str(CurrentHighest) + '%' +
                  (' ' * 22))
